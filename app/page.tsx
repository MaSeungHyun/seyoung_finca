...
export default function Home() {
  ...
  return (
    <div className="p-3 min-w-screen min-h-screen">
      <FincaHeader/>
      <Finca src={main} title={'Home'} subTitle={"가장 개인적인 공간에서 취하는 창의적인 휴식"}/>
      <div className="flex w-screen justify-center">
      {images.map((item)=> {
        return <FincaItem src={item.src} sale={item.sale} price={item.price}/>
      })}
      </div>
      <Finca src={pillow} title={"Pillow"} subTitle={"라이프스타일에 따라 패턴부터 소재까지 셀랙해보세요"} style={{flexDirection:'row-reverse'}} textStyle={{alignItems:'end'}}/>
